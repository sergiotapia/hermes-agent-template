...
def next_appointment(args: dict, **kwargs) -> str:
    ...
    hermes_session_id = str(hermes_session_id).strip()
    ...
    body = json.dumps({"hermes_session_id": hermes_session_id}).encode("utf-8")
    request = urllib.request.Request(
        f"{base_url}/api/internal/hermes/care-agent/tools/next-appointment",
        data=body,
        headers={
